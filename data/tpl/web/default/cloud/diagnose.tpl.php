...
<div class="main form-horizontal form">
	...
	<div class="form-group">
		<label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">绑定云服务接口IP</label>
		<div class="col-sm-10">
			<p class="form-control-static"><?php  if(!empty($_W['setting']['cloudip'])) { ?>自定义接口IP：<?php  echo $_W['setting']['cloudip']['ip'];?><?php  } ?>&nbsp;<a href="javascript:;"  data-toggle="modal" data-target="#set-server-ip">设置接口IP</a></p>
		</div>
	</div>
	<div class="form-group">
...
<script>
	...
	$('.js-checkip p').each(function(){
		...
		$.getJSON('./index.php?c=cloud&a=diagnose&do=testapi&ip='+$this.find('#serverdnsip').html(), function(testdata) {
			$this.find('#checkresult').html(testdata.message);
		});
	});
	$.ajax({
		type : "get",
